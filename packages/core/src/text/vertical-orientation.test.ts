...
describe('verticalBracketFormSubstitute (UAX #50 Tr code points → U+FE1x/FE3x vertical forms)', () => {
  ...
  it('verticalTrLongMark identifies the three probe-routed long marks without implying a fallback transform', () => {
    expect(verticalTrLongMark(0x30fc)).toBe(true); // ー prolonged sound mark
    expect(verticalTrLongMark(0x301c)).toBe(true); // 〜 wave dash
    expect(verticalTrLongMark(0xff5e)).toBe(true); // ～ fullwidth tilde
    expect(verticalTrLongMark(0x201c)).toBe(false); // “ left double quote
    expect(verticalTrLongMark(0xff1a)).toBe(false); // ： fullwidth colon
    expect(verticalTrLongMark(0x300c)).toBe(false); // 「 substituted bracket
  });
