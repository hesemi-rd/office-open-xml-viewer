...
- **diff**: new `@silurus/ooxml/diff` entrypoint exporting `diffPptx` / `diffDocx` / `diffXlsx`, which produce a structural `Change[]` between two parsed documents — slide / page / paragraph / cell level, with locations (slide index + EMU bbox for PPTX, paragraph index for DOCX, A1-style cell ref for XLSX). PPTX element matching uses parser-emitted `id` / `name` as the primary key with greedy nearest-neighbour by type + position as fallback (CHANGELOG 0.32.0 introduced these fields). DOCX paragraph alignment uses LCS over text signatures so single-paragraph edits stay tagged as `modify` instead of remove + add. Repeated cNvPr ids on the same slide (PowerPoint reuses them across shapes) are handled by consuming indices in order rather than overwriting.
...
- **export**: new PNG export helpers on `PptxPresentation` / `DocxDocument` (`exportSlideToPng`, `exportAllSlidesToPng` for PPTX; `exportPageToPng`, `exportAllPagesToPng` for DOCX) plus convenience wrappers on `PptxViewer` / `DocxViewer`. **Zero runtime dependencies** — pages render to an off-screen `HTMLCanvasElement` and encode via `canvas.toBlob('image/png')`. Pixel size is configurable via `{ width, dpr }`. Multi-page PDF stitching is intentionally left to the caller to preserve the zero-dep guarantee; the `PageBitmap` records emitted by `@silurus/ooxml-core`'s `renderAllPagesToPng` carry `pointWidth` / `pointHeight` (PDF points derived from EMU for PPTX, `<w:sectPr>` for DOCX) so callers can hand them straight to `pdf-lib` / jsPDF in user code.
...
- **storybook**: new `PptxViewer/Diff`, `DocxViewer/Diff`, `XlsxViewer/Diff` story groups demonstrating side-by-side diff with file upload, plus a new `PptxViewer/Export` story that downloads PNG / PDF outputs.
