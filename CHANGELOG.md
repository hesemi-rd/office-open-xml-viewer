...
- **export**: new PNG / PDF export helpers on `PptxPresentation` / `DocxDocument` (`exportSlideToPng`, `exportAllSlidesToPng`, `exportToPdf` for PPTX; `exportPageToPng`, `exportAllPagesToPng`, `exportToPdf` for DOCX) plus convenience wrappers on `PptxViewer` / `DocxViewer`. PDF assembly uses `pdf-lib` (lazy-imported, so bundles that never export pay no cost). Pages are rendered to an off-screen `HTMLCanvasElement` so the visible viewer is not disturbed. Pixel size is configurable via `{ width, dpr }`; page size in points is derived from EMU (PPTX) or section (DOCX). Shared primitives live under `@silurus/ooxml-core/export`.
